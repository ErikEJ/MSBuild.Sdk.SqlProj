# Publishing SQL Database Projects as Container Images with MSBuild.Sdk.SqlProj
...
Scripts that need to run before or after the main schema deployment (for example, to seed reference data or migrate legacy data) are supported through dedicated `PreDeploy` and `PostDeploy` item types. This keeps imperative data migration logic separate from the declarative schema definition.

### SQLCMD Variables

Deployment-time configuration such as database names, server names, and environment-specific settings can be parameterized using SQLCMD variables, exactly as in `.sqlproj` projects.
...
Starting with MSBuild.Sdk.SqlProj version 4.0.0, you can publish your database project as a runnable container image using `dotnet publish /t:PublishContainer`. The image bundles both [SqlPackage](https://learn.microsoft.com/en-us/sql/tools/sqlpackage/sqlpackage) and your `.dacpac` file, so everything needed to deploy the schema is self-contained inside the image.
...
MSBuild.Sdk.SqlProj supports two publishing modes:

| Approach | Command | Best For |
|---|---|---|
| Direct deploy to SQL Server | `dotnet publish /t:PublishDatabase` | Local development, quick iterations |
| Container image | `dotnet publish /t:PublishContainer` | CI/CD pipelines, repeatable deployments, multi-environment |

For most teams, we recommend the container image approach for pipeline deployments because it is more reproducible and does not require any SQL tooling on the deployment agent. For local development, the direct deploy approach remains convenient.
...
For more details on all available features, see the [MSBuild.Sdk.SqlProj README](https://github.com/ErikEJ/MSBuild.Sdk.SqlProj/blob/master/README.md).
